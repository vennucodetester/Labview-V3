...
class DataManager(QObject):
    ...
    def load_csv(self, file_path):
        ...
        try:
            ...
            new_csv_data.columns = self._normalize_column_names(new_csv_data.columns)
            ...
            if self.config_path and self.config_sensor_list:
                ...
                self.prune_graph_sensors_to_visible_csv(reason="CSV load")
    ...
    def reconcile_csv(self, new_csv_data, new_sensor_list):
        ...
        if dialog.exec():
            ...
            self.prune_graph_sensors_to_visible_csv(reason="CSV reconciliation")
    ...
    def load_session(self, file_path):
        ...
        try:
            ...
            self.case_id = session_data.get('caseId')
            self.refrigerant = session_data.get('refrigerant', self.refrigerant)
            ...
            cleaned_roles = self._normalize_sensor_roles_values(self.diagram_model)
            if cleaned_roles:
                print(f"[MIGRATE] Trimmed whitespace from {cleaned_roles} sensor role mapping value(s)")
            ...
            self.prune_graph_sensors_to_visible_csv(reason="session load")
    ...
    def selected_sensor_needs_diagram_assignment(self) -> bool:
        """True when a selected CSV column has no visible diagram dot yet."""
        if self.csv_data is None:
            return False
        try:
            csv_sensors = set(self.get_sensor_list())
        except Exception:
            csv_sensors = set()
        for sensor_name in self.selected_sensors:
            if sensor_name in csv_sensors and not self.is_sensor_mapped_to_visible_role(sensor_name):
                return True
        return False

    def learn_sensor_alias(self, canonical: str, csv_name: str):
        """Persist a CSV column name as an alias for a canonical diagram sensor."""
        canonical = (canonical or "").strip()
        csv_name = (csv_name or "").strip()
        if not canonical or not csv_name:
            return
        self._save_learned_alias(canonical, csv_name)
        print(f"[ALIAS_DB] Learned alias '{csv_name}' -> {canonical}")
    ...
    def _visible_csv_graph_keys(self) -> set:
        """Exact sensor keys that can be checked in the CSV-driven panel."""
        if self.csv_data is not None:
            return set(self.get_sensor_list())
        if self.config_sensor_list:
            return set(str(sensor).strip() for sensor in self.config_sensor_list if str(sensor).strip())
        return set()

    def prune_graph_sensors_to_visible_csv(self, reason: str = "") -> int:
        """Drop saved graph selections that no visible CSV row can show."""
        allowed = self._visible_csv_graph_keys()
        if not allowed:
            return 0

        before = set(str(sensor).strip() for sensor in self.graph_sensors if str(sensor).strip())
        kept = before & allowed
        dropped = before - kept
        if not dropped and kept == self.graph_sensors:
            return 0

        self.graph_sensors = kept
        if dropped:
            message = f"Dropped {len(dropped)} stale graph selection(s)"
            if reason:
                message += f" after {reason}"
            print(f"[GRAPH] {message}: {', '.join(sorted(dropped)[:10])}")
            try:
    ...
    def get_expected_sensor_rows(self, include_disabled: bool = False) -> list:
        ...
        for role_key in self._enumerate_diagram_role_keys():
            ...
            rows.append({
                ...
                "instrument_group": self.instrument_card_for_canonical(canonical),
                "enabled": enabled,
    ...
    def _group_for_canonical(canonical: str) -> str:
        if canonical.startswith(("T_amb", "T_wall", "T_ceil")):
            return "Ambient & Room"
        if DataManager.instrument_card_for_canonical(canonical):
            return DataManager.instrument_card_for_canonical(canonical)
    ...
    @staticmethod
    def instrument_card_for_canonical(canonical: str) -> str:
        """Return the Instrument Panel card name for an off-process canonical."""
        ...
        if canonical.startswith(("t_run", "f_defrost", "f_alwaysoff",
                                 "m_dot_meas", "gpm", "rpm")):
            return "System & Flags"
    ...
    def get_sensor_number(self, sensor_name):
        ...
        if self.csv_data is None and not self.config_sensor_list:
            return None
    ...
    def _sanitize_columns(self, df):
        """
        Strip labels and remove Unnamed/empty columns from a DataFrame.
    ...
    def _normalize_column_names(columns):
        """Trim column labels once and keep names unique after trimming."""
        normalized = []
        seen = {}
        for col in columns:
            ...
            seen[name] = count + 1
            normalized.append(name if count == 0 else f"{name}.{count}")
        return normalized

    @staticmethod
    def _normalize_sensor_roles_values(diagram_model: dict) -> int:
        roles = (diagram_model or {}).get('sensor_roles')
        if not isinstance(roles, dict):
            return 0
        changed = 0
        for role_key, sensor_name in list(roles.items()):
            if isinstance(sensor_name, str):
                stripped = sensor_name.strip()
                if stripped != sensor_name:
                    roles[role_key] = stripped
                    changed += 1
        return changed
    ...
    def _sanitize_sensor_list(self, sensor_list):
        ...
        for sensor in sensor_list:
            name = str(sensor).strip() if sensor is not None else ''
    ...
    def load_session_from_dict(self, session_data):
        ...
        try:
            ...
            cleaned_roles = self._normalize_sensor_roles_values(self.diagram_model)
            if cleaned_roles:
                print(f"[MIGRATE] Trimmed whitespace from {cleaned_roles} undo sensor role mapping value(s)")
            ...
            self.prune_graph_sensors_to_visible_csv(reason="undo/session restore")
    ...
    def auto_map_csv_to_canonical(self, csv_columns) -> int:
        ...
        for canonical, role_keys in list(canon_to_role_keys.items()):
            canon_to_role_keys[canonical] = sorted(
                role_keys,
                key=lambda rk, c=canonical: self._role_assignment_sort_key(c, rk)
            )
    ...
    def _role_assignment_sort_key(self, canonical: str, role_key: str):
        ...
        text = f"{canonical} {role_key}".lower()
        numbers = [int(n) for n in re.findall(r'\d+', text)]
        primary_number = numbers[-1] if numbers else 9999
        circuit_rank = 99
        for idx, token in enumerate(("left", "lh", "center", "ctr", "right", "rh")):
            if token in text:
                circuit_rank = idx
                break
        visible_rank = 0 if self.is_visible_diagram_role(role_key) else 1
        return (primary_number, circuit_rank, visible_rank, role_key)
    ...
    def get_csv_sensor_rows(self) -> list:
        """Rows for the sensor panel after a CSV is loaded: CSV columns first."""
        if self.csv_data is None:
            return []
        ...
        for row in self.get_expected_sensor_rows(include_disabled=True):
            ...
            expected_by_role[role_key] = row
            mapped = roles.get(role_key)
            if mapped:
                expected_by_sensor.setdefault(mapped, row)

        rows = []
        for col in csv_columns:
            expected = expected_by_sensor.get(col)
            if expected:
                home = self.sensor_home(col, expected_by_sensor=expected_by_sensor)
                group = expected.get('group') or 'Mapped Sensors'
                if home == 'instrument':
                    group = expected.get('instrument_group') or group or 'Other Instruments'
                rows.append({
                    "csv_column": col,
                    "display_label": expected.get('default_label') or expected.get('human_label') or col,
                    "sensor_number": self.get_sensor_number(col),
                    "role_key": expected.get('role_key') or '',
                    "canonical": expected.get('canonical') or '',
                    "human_label": expected.get('human_label') or '',
                    "group": group,
                    "mapped": True,
                    "home": home,
                    "on_diagram": home == 'process',
                    "not_on_diagram": home is None,
                })
            else:
                rows.append({
                    "csv_column": col,
                    ...
                    "canonical": '',
                    "human_label": '',
                    "group": "Not on diagram",
                    "mapped": False,
                    "home": None,
                    "on_diagram": False,
                    "not_on_diagram": True,
                })
        rows.sort(key=lambda r: (r.get("group") == "Not on diagram", r.get("group") or "", self._sensor_sort_key(r.get("canonical") or "", r.get("display_label") or "")))
        return rows

    def sensor_home(self, column: str, expected_by_sensor: dict | None = None):
        """Classify a CSV column as process, instrument, or truly homeless."""
        if not column:
            return None
        roles = self.diagram_model.get('sensor_roles') or {}
        if expected_by_sensor is None:
            expected_by_sensor = {}
            for row in self.get_expected_sensor_rows(include_disabled=True):
                mapped = roles.get(row.get('role_key') or '')
                if mapped:
                    expected_by_sensor.setdefault(mapped, row)
        row = expected_by_sensor.get(column)
        if not row:
            return None
        role_key = row.get('role_key') or ''
        if self.is_instrument_role(role_key):
            return 'instrument'
    ...
    def is_instrument_role(self, role_key: str) -> bool:
        """True when role_key belongs to the off-process Instrument Panel."""
        return bool(role_key) and str(role_key).lower().startswith('sensorbox.')

    def sensor_mapping_role_kinds(self, sensor_name: str) -> set:
        """Return {'diagram', 'instrument'} kinds for a mapped CSV sensor."""
        kinds = set()
        try:
            roles = self.diagram_model.get('sensor_roles', {}) or {}
            for role_key, mapped in roles.items():
                ...
                if self.is_visible_diagram_role(role_key):
                    kinds.add('diagram')
                elif self.is_instrument_role(role_key):
