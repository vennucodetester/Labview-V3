...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QPushButton, QHBoxLayout, QFrame,
                             ...
                             QLineEdit, QComboBox, QFormLayout, QApplication)
...
class GraphDateAxis(pg.DateAxisItem):
    """Bottom time axis with direct wheel/double-click gestures."""
    ...
    def mouseDoubleClickEvent(self, event):
        self.owner.fit_x_to_data()
        event.accept()

    def hoverEnterEvent(self, event):
        self.setPen(pg.mkPen('#0072B2', width=2))
...
class GraphValueAxis(pg.AxisItem):
    """Left value axis with direct wheel/double-click gestures."""
...
class GraphViewBox(pg.ViewBox):
    """Plot-area gestures: pan, axis-aware wheel zoom, double-click fit."""
    ...
    def mouseDragEvent(self, event, axis=None):
        if event.button() == Qt.MouseButton.RightButton:
            try:
                if event.isFinish():
                    p1 = self.mapSceneToView(event.buttonDownScenePos())
                    p2 = self.mapSceneToView(event.scenePos())
...
class GraphWidget(QWidget):
    ...
    def setupUi(self):
        ...
        self.fit_btn.setToolTip("Fit the full graph")
        ...
        tools_menu = QMenu(self.tools_btn)
        self.keep_range_action = tools_menu.addAction("Keep Time Range...")
        self.delete_range_action = tools_menu.addAction("Delete Time Range...")
        self.apply_range_action = tools_menu.addAction("Apply Selected Range(s)")
        self.apply_range_action.setEnabled(False)
        tools_menu.addSeparator()
        self.auto_detect_action = tools_menu.addAction("Auto-Detect Defrost...")
        self.sensor_ranges_action = tools_menu.addAction("Sensor Ranges...")
        self.tools_btn.setMenu(tools_menu)

        control_layout.insertWidget(0, self.fit_btn)
        control_layout.insertWidget(1, self.snapshot_btn)
        control_layout.insertWidget(2, self.tools_btn)
        ...
        self.view_box = GraphViewBox(self)
        self.left_axis = GraphValueAxis(self)
        self.bottom_axis = GraphDateAxis(self)
        self.plot_widget = pg.PlotWidget(
            axisItems={'left': self.left_axis, 'bottom': self.bottom_axis},
    ...
    def connect_signals(self):
        ...
        self.keep_range_action.triggered.connect(lambda: self.start_range_selection('keep'))
        self.delete_range_action.triggered.connect(lambda: self.start_range_selection('delete'))
        self.apply_range_action.triggered.connect(self.apply_custom_range)
        self.auto_detect_action.triggered.connect(self.auto_detect_defrost)
        self.sensor_ranges_action.triggered.connect(self.open_sensor_ranges)

    def reset_zoom(self):
        self.plot_widget.autoRange()

    def open_sensor_ranges(self):
        """Open the shared sensor range editor from the Graph tools menu."""
        mw = self.window()
        if hasattr(mw, 'open_range_editor_dialog'):
            mw.open_range_editor_dialog()

    def _event_wheel_delta(self, event) -> float:
        try:
    ...
    def zoom_axes_at(self, axes, event):
        ...
        if delta == 0:
            return
        scale = 0.88 if delta > 0 else 1 / 0.88
        x_range, y_range = vb.viewRange()
        if 'y' in axes:
            lo, hi = y_range
            anchor = point.y() if point is not None else (lo + hi) / 2
            new_lo = anchor - (anchor - lo) * scale
            new_hi = anchor + (hi - anchor) * scale
            vb.setYRange(new_lo, new_hi, padding=0)
        if 'x' in axes:
            lo, hi = x_range
            anchor = point.x() if point is not None else (lo + hi) / 2
            new_lo = anchor - (anchor - lo) * scale
            new_hi = anchor + (hi - anchor) * scale
            vb.setXRange(new_lo, new_hi, padding=0)

    def fit_x_to_data(self):
        """Fit the X axis to plotted data without changing Y."""
        lo = hi = None
        for item in self.plot_widget.listDataItems():
            try:
                bounds = item.dataBounds(0)
            except Exception:
                bounds = None
        ...
        if hi <= lo:
            ...
            lo, hi = lo - pad, hi + pad
        self.plot_widget.setXRange(lo, hi, padding=0.02)
    ...
    def _resolve_graph_sensors(self, selected_sensors, df):
        """Return checked graph sensors that are exact dataframe columns."""
        if df is None:
            return [], []
        ...
        for raw in sorted((str(sensor) for sensor in selected_sensors), key=str.lower):
            if raw in exact_columns and raw not in seen:
                resolved.append(raw)
                seen.add(raw)
            elif raw not in exact_columns:
    ...
    def update_ui(self):
        ...
        if df is not None:
            self.data_manager.prune_graph_sensors_to_visible_csv(reason="graph update")
    ...
    def start_range_selection(self, mode):
        ...
        if hasattr(self, 'apply_range_action'):
            self.apply_range_action.setEnabled(True)
